...
**Output layer:** INTERO — interoceptive inference, body-state signal resolution, visceral regulation, embodied selfhood
...
## The Complete Chain
```
Cognitive model of body state (prior)
→ Descending interoceptive predictions
→ Comparison with ascending visceral signals (NTS → insula → ACC)
→ Prediction error generated
→ Three pathways:
PATH A (Healthy updating):
Prediction error → precision-weighted belief revision
→ Prior updates toward actual body state
→ Accurate interoceptive awareness
→ Correct regulatory response
PATH B (Active inference):
Prediction error → motor/autonomic output
→ Body state changes to match prior
→ Prediction error minimized through action
→ Regulatory homeostasis maintained
PATH C (Failure mode):
Prediction error ignored (low precision weighting on signals)
→ Prior maintained despite contradicting evidence
→ Prior generates symptoms directly
→ Regulatory responses to non-existent states
→ FND, chronic pain, medically unexplained symptoms
```
---
...
**Hyperprecise prior:** The system is absolutely certain about what the body should be doing. Any deviation reads as high-amplitude prediction error requiring urgent response. Panic disorder — the prior says cardiac threat imminent, normal heart rate variation reads as emergency.
...
|---|---|---|---|
| **FND** | Prior generates symptoms without tissue signal | Maximum prior dominance — precision of ascending signals near zero | Yes — origin of the framework |
| **Chronic pain / fibromyalgia** | Hyperprecise threat prior at high gain | Central sensitization = prior locked on pain expected, normal signals read as threat | Yes — derived before confirmed |
| **Panic disorder** | Hyperprecise cardiac/respiratory prior | Normal interoceptive variation reads as emergency, cascade confirms prior | Yes |
| **PTSD** | Context rigidity — prior cannot update across contexts | Threat prior cannot update in safe environments | Yes |
| **Depression** | Noisy signals → prior dominance → homeostatic set-point drift | Low precision → system relies on low-energy prior → fatigue and anhedonia generated by prior | Yes |
| **Eating disorders** | Hyperprecise prior about body state | Distorted body-state prior generates persistent mismatch | Yes |
| **Addiction** | Active inference locked on drug-state prior | Prior says normal state = drug state, every non-drug moment generates prediction error | Yes |
| **Alexithymia** | Low interoceptive signal access | Ascending signals not reaching cortical awareness — prior disconnected from signal | Yes |
| **Autism (interoceptive)** | High-precision signals, impaired integration | Too much signal detail without adequate filtering — overwhelm rather than deficit | Yes |
...
|---|---|---|
| Slow breathing | Generates new interoceptive signals → active inference toward lower arousal prior | Prior recalibration through action — most direct intervention |
| Mindfulness | Reduces prior precision weighting → present-moment signals update model | Body predictions dissipate as model is driven by incoming sensory input rather than prior |
| Psychedelics | Collapse precision weighting globally → prior dissolves | Maximum updating window — therapeutic for PTSD and chronic pain |
| Exposure therapy | Repeated signal-prior mismatch in safe context → context rigidity reduction | Systematically recalibrates prior with high-precision safe-context signals |
| Opioids | Suppress ascending interoceptive signal amplitude | Reduce signal reaching prior — prediction error decreases but prior remains uncorrected |
...
Interoceptive accuracy measures should correlate with treatment response across psychiatric diagnoses as a predictor of whether the precision weighting mechanism is functional enough to support belief updating. Low interoceptive accuracy predicts treatment resistance regardless of diagnosis.
**Prediction 2 — Chain completeness: Scaffolding**
Interventions targeting precision weighting directly should show transdiagnostic efficacy — improving outcomes across depression, anxiety, chronic pain, and eating disorders simultaneously — because they address the shared mechanism rather than diagnosis-specific symptoms.
**Prediction 3 — Chain completeness: Scaffolding**
HRV improvement should precede and predict improvement in interoceptive accuracy — because vagal tone determines quality of ascending signals available for precision weighting. The AUTO↔MOD→COG↔INTERO cascade should produce measurable sequence effects in treatment response.
...
**High-gain incomplete resolution → interoceptive prior capture:**

The perceptual inversion failure mode describes prior dominance that develops through chronic slow drift. A distinct acute mechanism operates in high-gain profiles through a faster route.

When anterior insula Glx is elevated and GABA tone is insufficient to suppress a high-amplitude prediction error cleanly, the error persists at sub-threshold activation — the MOD↔COG incomplete resolution mechanism. An unresolved high-amplitude error running continuously becomes the dominant input to the descending interoceptive prediction system. The prior begins updating toward an alert state — "this problem is unresolved, the body should be in a state of urgency" — because that is what it is continuously receiving.

Once the prior has updated toward that state, active inference takes over: the body is now generating physiological correlates of unresolved tension and urgency not from ascending signal but from the prior predicting that state. Sympathetic tone rises. Elevated sympathetic tone reduces GABAergic availability. Reduced GABA means the original prediction error suppresses even less cleanly. The error persists longer. The prior updates further. The loop tightens.

The critical feature of this failure mode is its speed relative to chronic prior drift. Chronic FND and rumination develop over timescales of months to years as the prior slowly recalibrates. High-gain interoceptive prior capture can complete within a single session of sustained high-precision engagement with an unresolved problem — hours, not years. The mechanism is the same; the gain floor sets the speed.

**The lock-in phenomenology:** Once the prior has captured the alert state, the COG↔INTERO perceptual inversion applies in full. The person does not experience "my prior is generating urgency." They experience the problem as genuinely urgent and unresolved — because the prior is now producing that signal with the same phenomenological salience as an ascending interoceptive signal would. This is not a reasoning error. The instrument that would distinguish prior-generated urgency from genuine ascending urgency is operating inside the loop the prior has already captured.

**Exit conditions:** Because the lock runs through sympathetic tone → GABA reduction → incomplete suppression → prior reinforcement, cognitive engagement with the problem cannot interrupt it. Attempting to solve the problem from inside the state adds more prediction error load to a system that cannot suppress errors cleanly. The loop's only entry point from below is sympathetic tone reduction — amplitude restoration, extended exhale, vagal activation — which restores GABA availability, which allows the pending error signals to suppress, which allows the prior to begin updating away from the alert state. The resolution of the problem itself may be irrelevant if it occurs before the physiological floor is restored: a correct solution generated from inside the loop may not register as resolution because the prior is generating the urgency signal independently of the problem's actual status.

**Chain completeness:** Prior dominance via active inference: load-bearing in COG↔INTERO. GABA incomplete suppression as the MOD↔COG input mechanism: load-bearing from the MOD↔COG GABA section. Sympathetic tone → GABA availability → incomplete suppression feedback: scaffolding — mechanistically coherent, direct measurement of GABA dynamics during sustained high-precision engagement in high-Glx profiles pending. Speed differential between chronic prior drift and acute high-gain capture: predictive — no direct comparison measurement exists.
...
- MOD ↔ COG: Precision gain calibration sets the baseline precision weighting available for interoceptive inference — if MOD is miscalibrated, the instrument that calibrates this contract is broken
- MOD ↔ COG: High-gain incomplete GABA suppression provides the unresolved error signal that initiates the acute prior capture failure mode — the MOD layer is the upstream input to this contract's fastest-onset failure pathway.**
- AUTO ↔ MOD: Vagal tone determines signal quality before it reaches this contract — low vagal tone means noisy ascending signals means prior dominance by default
**Above (fed by this contract):**
- INTERO ↔ DEV: The interoceptive prior established at this level becomes the setpoint input to the developmental layer — chronic prior miscalibration writes itself into the developmental baseline over time
**Lateral:**
- COG ↔ SOC: Social interoception — reading others' body states through co-regulation — uses the same precision weighting apparatus. A miscalibrated interoceptive prior affects not just self-sensing but other-sensing simultaneously.
---
## Origin Note
