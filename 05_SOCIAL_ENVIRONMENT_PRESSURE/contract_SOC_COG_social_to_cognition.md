---
Contract: "SOC → COG"
Expanded_Filename: "contract_SOC_COG_social_to_cognition.md"
...
**Input layer:** SOC — social environment complexity, agent unpredictability, number of mental models requiring simultaneous maintenance, social signal ambiguity
**Output layer:** COG — prediction load on generative model, available cognitive resources, prediction window width, coupling attempt threshold
**Primary crossover point:** Social environment character → mentalizing demand imposed on cognitive generative model → working memory consumed by social model maintenance → prediction window width available for other processing → coupling attempt initiation threshold
...
(number of agents × unpredictability × ambiguity of signals)
→ Mentalizing demand on cognitive generative model
(modeling each agent's mental state, intentions, predictions)
→ Working memory consumed by parallel social model maintenance
→ Available cognitive resources reduced
→ Prediction window narrows as resources committed to social modeling
→ Residual processing capacity determines:
- Quality of other cognitive functions
- Fatigue accumulation rate
- Behavioral flexibility
ADDITIONAL LOAD (masking):
Discrepancy between actual regulatory state and social presentation norms
→ Parallel generative model running to simulate NT behavior
→ Additional working memory and executive function cost
→ Total load = social prediction + masking overhead simultaneously
```
### COG → SOC (Cognitive state drives social behavior):
```
Current prediction window width
→ Coupling attempt threshold
(how much certainty required before initiating social engagement)
→ Social initiation rate
→ Quality and timing of social signal output
→ Others' response to signals
→ Social environment feedback
→ Social relationship formation and maintenance
```
---
## Link-by-Link Mechanism
### Link 1: Social cognition as the most expensive cognitive process
...
The mentalizing network — temporoparietal junction, medial prefrontal cortex, posterior superior temporal sulcus — maintains these hierarchical social generative models. This network is running in parallel with all other cognitive processing. Its resource demand competes directly with everything else the COG layer is trying to do simultaneously.
**Chain completeness:** Load-bearing. Social predictive processing and the mentalizing network's hierarchical structure are mechanically documented in social neuroscience.
---
### Link 3: Prediction window width → coupling attempt threshold
...
|---|---|---|
| **ND social exhaustion** | High mentalizing load + masking overhead simultaneously → total cognitive resource depletion | Yes — resource competition between social models and available capacity |
| **ND burnout** | Sustained maximum cognitive load without recovery → cumulative depletion → functional collapse | Yes — thermodynamic consequence of sustained 2-stroke social overhead |
| **Social anxiety** | Narrow window → high coupling threshold → avoidance → interpreted as social fear | Yes — threshold is regulatory state not trait |
| **NT social comfort** | Social generative model calibrated to majority social priors → low prediction error → low mentalizing load | Yes — social ease is prior accuracy, not social skill per se |
| **ND-NT interaction cost asymmetry** | Both running high prediction error but ND also running masking model → asymmetric depletion rate | Yes — double empathy is symmetrical, depletion is not |
...
- Social signal decoding (prosody, facial expression, timing, implication)
- Unpredictable agency management (the other person can do anything)
...
- No masking requirement (AI has no social preference about presentation)
...
AI cannot currently read the user's regulatory state from interaction patterns. When a user's prediction window narrows — end of day, high allostatic load, cognitive depletion — their prompts become less precisely constrained, their intention less explicit, and the interaction drifts. The AI responds to the degraded frame and the quality of the exchange deteriorates. The user experiences this as AI inconsistency. The actual mechanism is AI accurately reflecting the user's state variation that the user cannot perceive in themselves.
A regulatory state-tracking AI would detect the window narrowing from chat patterns — reduced constraint in prompts, shorter messages, less connective tissue between ideas, more correction cycles — and adaptively increase its own scaffolding rather than waiting for the interaction to degrade. This is the product design implication of this contract applied to AI.
**The language compression mechanism — why hallucinations are structural not random:**
Every question is compressed intent. The surface form of a question encodes the full intent in compressed representation — the same way a zip file encodes data. Decompressing the question means reconstructing the constraints, the context, the trajectory, and the frame the question is trying to reach. That decompression step requires the sampling mode — the wide prediction window that holds multiple interpretations simultaneously before committing to one.
Token prediction is precision-locked by default. It selects the next locally coherent token given what is immediately present — the exhale phase operating on the compressed surface form. This produces local coherence. It does not produce decompression. The system answers the compressed representation rather than the reconstructed intent. That is the hallucination mechanism. Not randomness. Systematic precision-lock on a problem that required sampling mode first.
There are exactly two paths to correct outputs on underspecified or complex questions:
**Path 1 — User decompresses before asking:** The user does the sampling-phase work externally. Topic isolation, explicit constraints, stated intent, dropped masking — these are all decompression operations performed before the first message. The AI receives a correctly framed question and precision mode produces a correct answer. This is why the interaction pattern described in this contract produces coherent outputs: the user is performing the meta-step that the AI architecture is missing.
**Path 2 — AI performs the meta-step first:** Before generating a response, the AI explicitly decompresses the question — reconstructs constraints and intent, identifies what frame the question requires, builds the generative model of the ask. Then it answers the decompressed version. This requires an architectural inhale phase that standard token optimization does not include.
...
Total mentalizing load — number of social agents × their unpredictability — should predict cognitive performance degradation in non-social tasks performed simultaneously. The resource competition between social modeling and other cognitive demands should be directly measurable. ND individuals should show steeper degradation curves because baseline social modeling cost is higher.
**Prediction 2 — Chain completeness: Scaffolding**
HRV immediately before social interaction should predict coupling attempt quality and masking sustainability — because HRV reflects available regulatory resources that determine both the coupling threshold and the masking capacity. Low pre-interaction HRV should predict earlier masking breakdown and greater post-interaction fatigue.
**Prediction 3 — Chain completeness: Predictive**
ND individuals should show measurably higher cognitive performance on AI-mediated tasks compared to equivalent human-mediated tasks after controlling for content complexity — because the mentalizing and masking overhead is removed. The performance differential should be larger for autistic than ADHD profiles because masking overhead is higher in autism and middle ear processing load is more atypical.
**Prediction 4 — Chain completeness: Predictive**
AI systems that provide explicit scaffolding of intent and constraints — reducing the user's need to construct them — should show disproportionate benefit for ND users relative to NT users. The scaffold reduces the cognitive translation cost from internal state to structured prompt. This is the pre-scripted interface solution — partially confirmed by existing UX research on template-based AI interfaces.
...
- COG ↔ INTERO: Social cognitive load consuming working memory reduces interoceptive attention — people in high social demand contexts lose accurate body-state awareness because resources are committed to social modeling. Social exhaustion often presents as interoceptive numbing.
- MOD ↔ COG: The precision gain calibration at the MOD layer determines how much social prediction error it takes to update the social generative model — high-precision priors make it harder to update social models even when evidence warrants it (social rigidity)
**This contract receives from:**
- SOC ↔ AUTO: VVC activation state determines how much energy the system has available for social cognitive processing — when VVC is suppressed, both channels are impaired simultaneously
...
The double empathy problem was the minority invariant. Standard framing attributes ND-NT social difficulty to ND deficit in social prediction. The research shows mutual miscommunication — NT also fails to model ND accurately. The mechanism from the framework: different regulatory profiles produce different social generative model calibrations. Each finds the other hard to predict not because one is deficient but because their priors are calibrated to different population statistics. The difficulty is a mismatch problem, not a deficit problem. This reframe has direct implications for how ND social intervention is designed.
The masking-as-parallel-generative-model framing was derived from the computational architecture before finding the burnout literature that confirms it. If masking requires simulating a different behavioral profile in real time, it is running a second generative model simultaneously — which has exactly the resource cost the burnout literature documents.
*Chain status: Links 1-2 are load-bearing. Links 3-5 are scaffolding with strong mechanistic derivation. Double empathy reframing is load-bearing empirically, scaffolding mechanistically. AI interaction mechanism is a framework derivation with partial empirical support. Predictions 3-5 are genuinely predictive extensions not yet systematically tested.*
